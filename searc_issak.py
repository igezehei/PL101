...
class Task(search.Problem):
    ...
    def get_new_action(self, action,  state):
        ...
        for c in para_combinators:
            ...
            for e in c:
                e_set.add(e)
            para_combinators.remove(c)
            for d in para_combinators:
                d_set = set()
                for g in d:
                    d_set.add(g)
                if self.compare_sets(e_set, d_set):
                    para_combinators.remove(d)
            para_combinators.append(temp)
        """
        """
        for c in para_combinators:
            print c
...
def depth_first_graph_search(problem):  
    ...
    while fringe:
        ...
        if node.state not in closed:
            ...
            new_node = [search.Node(next,node, action,()) for (action, next) in problem.successor(node.state)]
            fringe.extend(new_node)
    #return search.depth_first_graph_search(problem)
