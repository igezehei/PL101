    ...
    def __init__(self, name, precondition, effects):
        ...
        for condition, effect in effects:
            if not effect.negated:
                self.add_effects.append(effect)
            else:
                self.del_effects.append(effect)
   
        contradictions =[]
        for effect in self.del_effects:
            if effect.negate() in self.add_effects:
                contradictions.append(effect)
        self.del_effects = [effect.negate() for effect in self.del_effects if effect not in contradictions]
...
class State:
    ...
    def __str__(self):
    	return "State: %s (Domain: %s, Problem: %s)" % (self.__class__.__name__, self.domain.domain_name,self.problem.problem_name)
    def __repr__(self):
	return str(self)
...
class Task(search.Problem):
    ...
    def successor_assist(self, action, state):
         ...
         types = state.domain.types
         constants = state.domain.constants
         predicates = state.domain.predicates
         actions =state.domain.actions
         
         new_domain = Domain(domain_name, requirements,  types,  constants,  predicates, actions)
         
         problem_name =  state.problem.problem_name
         domain_name_2 = state.problem.domain_name
         requirements_2 = state.problem.requirements
         objects= state.problem.objects
         init = copy.deepcopy(state.problem.init)
         goal = state.problem.goal
         
         new_problem = Problem(problem_name,  domain_name_2, requirements_2, objects, init, goal)
         
         new_state = State(new_domain,  new_problem)
        """
    ...
    def get_new_action(self, action,  state):
        ...
        # To delete the cominations which have duplicated TypedObjects.
        del_list = []
        for c in para_combinators:
            label=False
            for element in c:
                cout = c.count(element)
                if cout >1:
                    label = True
                    break
            if label == True:
    ...
    def  goal_test(self,  state):
        is_goal = False
        for part in self.goal.parts:
            if part not in state.problem.init:
                return False
        is_goal = True
        return is_goal
    # The heuristic function for the informed search algorithm.
    # Define the distance: computing the number of  the parts of the goal which are not 
    # included in the init of the problem from the state stored in the given node.
    def h(self, node):
        cout =0
        for part in self.goal.parts:
            if part not in node.state.problem.init:
                cout +=1
        return cout
bfs_count = 0 # count the number of nodes traversed for bfs.
bfs_num = 0 # count the maximum number of nodes held in memory at any time for bfs
#Breadth first graph search algorithm
def breadth_first_graph_search(problem):
    global bfs_count
    global bfs_num
    initial_state = problem.initial
    fringe = utils.FIFOQueue()
    fringe.append(search.Node(initial_state))
    bfs_num = fringe.__len__()
    closed = {}
    while fringe:
        node = fringe.pop()
        bfs_count +=1
        ...
        if node.state not in closed:
            ...
            new_node =[]
            new_node = [search.Node(next,node, action,()) for (action, next) in problem.successor(node.state)]
            fringe.extend(new_node)
            if fringe.__len__()>bfs_num:
                bfs_num = fringe.__len__()
...
def f_uniform_cost(node):
    return node.path_cost
#Uniform cost search algorithm
def uniform_cost_search(problem):
    return search.graph_search(problem, utils.PriorityQueue(min, f_uniform_cost))
...
def parse_domain(domain_pddl):
    ...
    if  types_option[0]==':typing' or types_option[0]==':types':
...
if __name__ == "__main__":
    # To assure the command line include 7 argvs
    if len(sys.argv)!=7:
        print "Command line input error!"
        sys.exit()
    domainFile = sys.argv[2]
    problemFile = sys.argv[4]
    #To load the problem
    task= loadIntoProblem(domainFile,problemFile)
    result = None
    alg_label =""
    # Using one of the search algorithm
    try: 
        algorithm = sys.argv[6]
        if algorithm == "bfs":
            result= breadth_first_graph_search(task) 
            alg_label="breadth first graph search"
        elif algorithm =="dfs":
            result = depth_first_graph_search(task)
            alg_label="depth first graph search"
        elif algorithm =="ids":
            result = iterative_deepening_search(task) 
            alg_label="iterative deepening search"
        elif algorithm =="ucs":
            result = uniform_cost_search(task)
            alg_label="uniform cost search"
        elif algorithm == "bfgs":
            result = best_first_greedy_search(task)
            alg_label="best first greedy search"
        elif algorithm == "astar":
            result = astar_search(task)
            alg_label="A* search"
    except UnreachableError:
        print "The search specified is not a correct one"
        sys.exit()
    #Create a new file named "solution.txt"
    filename = "solution.txt"
    FILE = open(filename,"w")
    # Write the result to the newly created file.
    if  result is not None:
        path = result.path()
        FILE.write("The solution to the problem using "+alg_label+ " algorithm is: " + '\n')
        FILE.write("\n")
        stack = utils.Stack()
        for p in path:
            stack.append(p)
        ...
        while stack:
            ...
            count+=1
            if count == 1:
                FILE.write("Initial State: No Action!"+ '\n')
            else:
                FILE.write(str(s .action) +'\n')
            FILE.write("State " + str(count-1)+":"+'\n')
            for atom in s .state.problem.init:
                FILE.write(str(atom) + '\n')
            FILE.write("\n")
            FILE.write("\n")
    else:
        FILE.write("There is no solution to the problem!"+ '\n')
    FILE.close()
